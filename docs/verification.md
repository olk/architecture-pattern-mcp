...
|---|---|---|---|---|
| L0 | ruff, mypy `--strict` (zuban), vulture, deptry | style, type errors, dead code, dependency hygiene | pre-commit + CI (`make lint static-typing deadcode depcheck`) | active |
| L1 | pytest | chosen-input behaviour, regressions; secret canary (no key leakage into prompts/logs/persisted state) | `make unit-tests`, `make verify-hypothesis-oracles` | active |
| L1b | Hypothesis (payload strategies) | MCP tool-call boundary robustness: malformed/nested/oversized payloads — structured errors only, no tracebacks | collected by `make verify-hypothesis-oracles` (advisory → blocking Phase 1) | harness active, generators expanding |
| L2 | Hypothesis | input-space properties: job-lifecycle interleavings vs shadow automaton (J-1..J-4), LLM-boundary timeout/retry discipline (E5), normalization idempotence/dedup (N-1..N-4) | `make verify-hypothesis-oracles` | active |
| L3 | mutmut + planted-bug gardens | tests-the-tests: tautological oracles, vacuous contracts/assertions; the garden is the vacuity authority | `make mutation-tests` (manual/nightly) | garden active; mutmut nightly |
...
| L7 | deptry, uv.lock pinning, pip-audit, import-inventory diff | supply chain: hallucinated/unused/vulnerable deps; slopsquatting defence (new package names need human decision) | `make depcheck`, `make verify-import-inventory`, `make verify-deps-audit` (advisory) | active |
...
2. `mutmut` over the Tier A/B/C modules (`make mutation-tests`); its mypy
...
(`make depcheck`), the import-inventory drift gate
...
| Target | Layer(s) | When |
|---|---|---|
| `make lint` / `make static-typing` / `make deadcode` / `make depcheck` | L0 | every commit (pre-commit + CI) |
| `make unit-tests` | L1 | every commit |
| `make verify-hypothesis-oracles` | L1b, L2, plus L4 traces / L5 conformance / L6 DST / L1 canary | pre-push + CI |
| `make mutation-tests` | L3 | manual / nightly |
