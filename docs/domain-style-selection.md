...
4. **Fusion + rerank**: relative_score fusion (per-leg min-max normalization; leg weights default to dense 0.7 / BM25 0.3, tunable via `RETRIEVAL_DENSE_WEIGHT` / `RETRIEVAL_BM25_WEIGHT`, sum-to-1 validated — see `docs/retrieval-fusion-modes.md`), then mandatory TEI cross-encoder rerank capped at `rerank_top_n=10` — `src/patterns/retriever.py` (`_ensure_fusion_retriever`, rerank block)
