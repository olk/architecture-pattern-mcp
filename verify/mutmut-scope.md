...
|---|---|---|---|---|
| `src/design_validation.py` *(new — design-integrity gate)* | `evaluate_design_integrity`, `_duplicate_id_violations`, `_relationship_violations`, `_event_violations`, `_api_contract_violations`, `render_integrity_violation`, `format_integrity_error` | L1 `test_design_validation_internals.py` (direct kill oracle: one family per DIV rule, locator index arithmetic, lexicographic ordering, byte-pinned renderers) + `test_adapters.py::TestDesignIntegrityFindings` (soft path) + `tests/schemas/test_schemas.py::TestArchitectureDesignResponseIntegrity` (enforcement seam) | the runtime cross-reference gate for the INV-2..5 family — a dropped membership check silently ships dangling designs out of GENERATE (the defect class the benchmark measures as `dangling_references`) | none — 94/94 killed (ratio 1.00 at the 2026-09-20 baseline regen) |
...
| `src/tools/_adapters.py` *(round 2)* | overview validation adapters, `MalformedArchitectureOverviewError` raising, caller-payload sub-record parsers (`_parse_relationship`, `_parse_event_contract`, `_parse_api_contract`), `design_integrity_findings` | L1 `test_adapters.py` (13 → 22 tests, both directions incl. "does not silently default"; **2026-09-20:** the three sub-record parsers graduated from unclassified to classified coverage — the pre-integrity tests only fed empty relationship/event/contract lists so the functions never ran. `TestSubRecordParsers` pins parsed values + schema defaults and kills all 161 of their mutants (81/44/36, ratio 1.00); `TestDesignIntegrityFindings` pins the soft DIV path) | the ERR_012 boundary between LLM output and tool responses | triage pending (guards/overview helpers) |
