...
| `TEI-1` | A TEI result body exists only for the OK verdict — errors are pure signals | `always` | `tei_fallback.fizz::TEI1_ResultOnlyOnOk` (FG-23 must violate) | n/a | planned | `test_fizz_conformance.py` |
| `RET-1` | A rerank error propagates — never a partial/un-reranked outcome | `always` | `tei_fallback.fizz::RET1_NoPartialOnRerankError` (FG-24 must violate) | n/a | planned | `test_fizz_conformance.py` |
| `FUS-1` | A REAL outcome always carries ≥ 1 resolved pattern (empty recall must fall back) | `always` | `retrieval_fusion.fizz::FUS1_RealHasPatterns` (FG-25 must violate) | n/a | planned | `test_fizz_conformance.py` |
| `FUS-2` | A REAL outcome always passed the `min_fusion_score` gate | `always` | `retrieval_fusion.fizz::FUS2_RealPassedFloor` (FG-26 must violate) | n/a | planned | `test_fizz_conformance.py` |
| `FUS-3` | Every fallback carries the `is_fallback` sentinel (never outranks a real match) | `always` | `retrieval_fusion.fizz::FUS3_FallbackTagged` (FG-27 must violate) | n/a | planned | `test_fizz_conformance.py` |
