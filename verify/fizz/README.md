...
| `P-1` | Pipeline attempt loop bounded, `attempts <= 3` | `always` | `pipeline_control.fizz::P1_AttemptBound` (FG-32) | n/a (no static contract) | `test_pipeline_properties.py::TestP1AttemptBound` *(2026-09-12)* | planned |
| `FP-2` | No stage advance after cancellation observed | `always` | `pipeline_control.fizz::FP2_CancelStopsPipeline` | n/a (no static contract) | `test_pipeline_properties.py::TestDL3CancelFreezes` (pre-cancel + mid-run token) *(2026-09-12)* | planned |
| `FP-3` | Stage order ANALYZE→GENERATE→EVALUATE→REFINE never violated | `always` | `pipeline_control.fizz::FP3_StageOrder` | n/a (no static contract) | n/a — order witness lives in the Workflow event machinery (`# spec-explains:` in-lène coverage via DL oracles over design_loop's generate→evaluate pairing) | planned |
| `FP-4` | Every run ends in exactly one terminal outcome | `always` | `pipeline_control.fizz::FP4_TotalOutcome` (FG-33) | n/a (no static contract) | `test_pipeline_properties.py::TestFPTerminalDiscipline` *(2026-09-12)* | planned |
...
| `FP-6` | A run only ends FAILED after exhausting its attempts | `always` | `pipeline_control.fizz::FP6_NoPrematureFailure` (FG-09 must violate) | n/a | `test_pipeline_properties.py::TestFPTerminalDiscipline` + `TestDL4MalformedContinues` *(2026-09-12)* | planned |
| `FP-7` | A run only ends COMPLETED after all stages ran | `always` | `pipeline_control.fizz::FP7_NoPrematureCompletion` (FG-10 must violate) | n/a | `test_pipeline_properties.py::TestFPTerminalDiscipline` *(2026-09-12)* | planned |
...
| `DL-2` | `best_score` never decreases (guarded update is the only writer) | `always` | `design_loop.fizz::DL2_BestScoreMonotone` (FG-28 must violate) | n/a | `test_pipeline_properties.py::TestDL2BestScoreGuard` *(2026-09-12)* | planned |
